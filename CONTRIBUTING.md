...
Thanks for taking a look. This is an early-stage hobby project (see `docs/PLAN.md` for the
...
this project; this file is just the process wrapper around them. `docs/context.md` is
optional further reading — a session-by-session log of bugs already found and fixed and
dead ends already tried and rejected, worth a skim before you re-try something that looks
like an obvious fix, but not required up front.
...
See `docs/ARCHITECTURE.md`'s "Build & verify" section for the full picture, including how
to force a reimport after changing importer code.

There's no unit test suite — verification is done with throwaway `SceneTree` GDScript
scripts run headlessly against your own mounted game copy. `docs/ARCHITECTURE.md` has an
example. Don't commit these scratch scripts.
...
- If your change touches something `docs/ARCHITECTURE.md` documents (a convention, a
  format quirk, a gotcha), update `docs/ARCHITECTURE.md` in the same PR. It goes stale
  fast otherwise.
